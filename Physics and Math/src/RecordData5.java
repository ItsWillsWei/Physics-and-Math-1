	...
	public void paintComponent(Graphics g) {
		if (draw) {
			...
			if (plotType == POINT) {
				...
				// g.drawRect((int) points[point].x, (int) ((y2 -
				// points[point].y)), 1, 1);
				//
				// }
...
//					Hs[corner] = r * Math.cos(mus[corner]) + r * Math.sin(mus[corner]) * Math.tan(theta / 180.0 * Math.PI)
//							- r * Math.sin(mus[corner]) + r * Math.tan(theta / 180.0 * Math.PI) * Math.cos(mus[corner]);
				}
				...
				double betar = beta*beta;//0.393872;// 0.4735144;//0.433692871036404;
										// //edge-on restitution coefficient's
										// square
				// update energy profile after collision
				double Er = (0.5 * m * Vx0 * Vx0 + 0.5 * m * Math.pow((Vy0 - g * tf), 2)
						+ 0.5 * (I - m * r * r) * dO * dO) * betar;
				O = mus[stop?lastCorner:0] - Math.PI/2;//-(Math.PI / 2 - (O + dO * tf)); // TODO//starting phyla for
													// gaining contact
	...
	public static void main(String[] args) {
		...
		double m = 0.0233;//0.0280;//0.0252;// ////0.0233;//0.0252;////square0.0252;//0.0233;//hex:0.0280;//square:0.0252;//
							// mass
		// double g = 9.81;// gravitational field
		double r = 0.011865;//0.01413252572;///0.017334722741;// ////0.011865;//0.01413252572;//0.017334722741;//cylinder//0.011865;//hexagon//0.01413252572;//square//0.017334722741;//
									// Radius of object
		...
		double beta = 1;//0.856370966258631;//0.656282789918262;// 0.830672294438214;//0.882069405158529;////0.626049;//0.6865166;//////1;//0.8;////1;//0.856370966258631;//
