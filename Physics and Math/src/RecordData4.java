...
public class RecordData4 extends Graph {

	// Declare global static variables
	public static List<Point> plotPoints;
	public static boolean draw;

	RecordData4(double x1, double x2, double y1, double y2) {
		// Calls graph
		super(x1, x2, y1, y2);
		draw = false;
...
//				}
		...
		while (x <= totalHorizontalDist && walking) {
			...
			if (modelType == 1 && x < totalHorizontalDist && !walking) {
			    ...
			    while (H >= 0 && x <= totalHorizontalDist){
			        ...
			        //Define h, L, K, H
			        double h = r*Math.cos(O)+r*Math.sin(O)*Math.tan(theta/180.0*Math.PI) + Vy0*tf - 1.0/2.0*g*tf*tf + Vx0*tf*Math.tan(theta/180.0*Math.PI);//Equation x
			    ...
			    double betar = 0.393872;//0.4735144;//0.433692871036404; //edge-on restitution coefficient's square
	...
	public static void main(String[] args) {
		...
		RecordData4 graph = new RecordData4(x1, x2, y1, y2);

		// #Define and initialize variables and constants
		//double Oinit = 0.08085;// 4.61 * Math.PI / 180.0; // Start at 4.61
		double Oinit = 0;//0.08085;//0.1135177;//0.1135177;						// degrees
		double t;// Time

		double m = 0.0233;//0.0252;//0.0280;////0.0233;//0.0252;////square0.0252;//0.0233;//hex:0.0280;//square:0.0252;// mass
		//double g = 9.81;// gravitational field
		double r = 0.011865;;//0.01217;//0.01156;//;//0.017334722741;//0.01413252572;////0.011865;//0.01413252572;//0.017334722741;//cylinder//0.011865;//hexagon//0.01413252572;//square//0.017334722741;// Radius of object
		...
		double beta = 1;//0.656282789918262;//0.830672294438214;//0.882069405158529;//0.856370966258631;//0.626049;//0.6865166;//////1;//0.8;////1;//0.856370966258631;//  Coefficient of restitution (wf/wi)
